...
(defn ts->julian-day [ts]
  (+ (/ ts 86400.0)
     epoch-julian-day))

(defn julian-day->ts [j]
  (* (- j epoch-julian-day)
     86400))
...
(defn sunset-sunrise-times [ts lat lon elevation]
  (let [j (ts->julian-day ts)
    ...
    [(cjc/from-long (long (* 1000 (julian-day->ts j-rise))))
     (cjc/from-long (long (* 1000 (julian-day->ts j-set))))]))

;;    Corresponding ~ to Veneto
;;    [45.485717 11.315627]

(sunset-sunrise-times 1707225141 45.485717 11.315627 0)
